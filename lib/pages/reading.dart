...
class _ReadingPageState extends State<ReadingPage> {
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 8),
                    const Text('页面布局', style: TextStyle(fontWeight: FontWeight.w700)),
                    ...
                    const SizedBox(height: 8),
                    const Text('滚动方向', style: TextStyle(fontWeight: FontWeight.w700)),
                    ...
                    const SizedBox(height: 8),
                    const Text('缩放', style: TextStyle(fontWeight: FontWeight.w700)),
                    Row(
                      children: [
                        IconButton(
                          tooltip: '缩小',
  ...
  List<_ReadingBookmarkNode> _extractBookmarks(dynamic document) {
    ...
    void visit(dynamic bookmark, List<_ReadingBookmarkNode> sink) {
      ...
      String title = '未命名书签';
      try {
        final dynamic maybeTitle = bookmark.title ?? bookmark.name ?? bookmark.text;
  ...
  Future<void> _showBookmarkSheet() async {
    ...
    if (_bookmarks.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('当前 PDF 没有可用目录/书签')),
      );
      return;
    }
    ...
    await showModalBottomSheet<void>(
      ...
      builder: (sheetContext) {
        Widget buildNode(_ReadingBookmarkNode node, int depth) {
          return Padding(
            ...
            child: ListTile(
              dense: true,
              leading: Icon(
                depth == 0 ? Icons.bookmark_outline : Icons.subdirectory_arrow_right,
              ),
              ...
              onTap: () {
                ...
                try {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text('该目录项无法跳转')),
                  );
                }
              },
            ),
        ...
        List<Widget> buildTree(List<_ReadingBookmarkNode> nodes, [int depth = 0]) {
          final items = <Widget>[];
          ...
          children: [
            ...
            Card(
              child: ListTile(
                ...
                subtitle: Text('${recentBook.title} · 第$recentPage页 · $_lastOpenedLabel'),
                trailing: FilledButton(
                  onPressed: () => _openReading(book: recentBook, page: recentPage),
                  child: const Text('继续'),
                ),
              ),
            ),
            ...
            Card(
              child: ListTile(
                leading: Icon(_nightMode ? Icons.dark_mode : Icons.light_mode_outlined),
        ...
        actions: [
          ...
          if (_isReading)
            IconButton(
              ...
              icon: Icon(_nightMode ? Icons.dark_mode : Icons.light_mode_outlined),
            ),
          if (!_isReading)
            IconButton(
              tooltip: '打开阅读器',
            ...
            Row(
              children: [
                if (searchResult != null) ...[
                  IconButton(
                    tooltip: '上一个结果',
                    onPressed: searchResult!.hasResult && searchResult!.currentInstanceIndex > 0
                        ? onPreviousSearch
                        : null,
