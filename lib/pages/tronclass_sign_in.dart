...
class _TronclassSignInPageState extends State<TronclassSignInPage> {
  ...
  void _addLog({
    ...
    final log = TronclassSignLogItem(
      time: DateTime.now(),
    ...
    _logs.insert(0, log);
    unawaited(TronclassSignLogStore().append(_serializeLog(log)));
  }

  Map<String, dynamic> _serializeLog(TronclassSignLogItem log) {
    return {
  ...
  void _selectActivity(Active active) {
    ...
    if (mode == 'qrcode') {
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => TronclassQrSignPage(rollcallId: active.id),
        ),
      ...
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => TronclassNumberSignPage(rollcallId: active.id),
        ),
      ).then((_) => _loadActivities());
      return;
    } else if (mode == 'radar') {
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => TronclassRadarSignPage(rollcallId: active.id),
        ),
      ).then((_) => _loadActivities());
      return;
    }
  ...
  Future<void> _openQrScanner() async {
    ...
    Active? targetActivity = _currentActivity;
    if (scannedRollcallId != null && scannedRollcallId.isNotEmpty) {
      var matched = false;
      if (targetActivity != null && targetActivity.id == scannedRollcallId) {
        matched = true;
      } else {
        for (final activity in _activities) {
          if (activity.id == scannedRollcallId) {
      ...
      if (!matched) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('该二维码不属于当前课程的签到会话')));
        return;
      }
      if (targetActivity != null) {
        _selectActivity(targetActivity);
      }
    }
    ...
    if (targetActivity != null && _currentMode == 'qrcode') {
      await _sign(targetActivity);
      return;
    }
    ...
    if (mounted) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('已解析二维码，但当前会话不是二维码签到')));
    }
  }
