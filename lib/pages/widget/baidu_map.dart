...
class BaiduMapWidget extends StatefulWidget {
  ...
  final BMFCoordinate? initialCoordinate;
...
class _BaiduMapWidgetState extends State<BaiduMapWidget> {
  ...
  void initState() {
    ...
    if (widget.initialCoordinate != null) {
      _currentPosition = widget.initialCoordinate;
      _markerPosition = widget.initialCoordinate;
      final address = widget.initialAddress?.trim();
      _locationInfo = address == null || address.isEmpty
          ? '已载入上次选择位置\n纬度：${widget.initialCoordinate!.latitude.toStringAsFixed(6)}, 经度：${widget.initialCoordinate!.longitude.toStringAsFixed(6)}'
          : '地址：$address\n纬度：${widget.initialCoordinate!.latitude.toStringAsFixed(6)}, 经度：${widget.initialCoordinate!.longitude.toStringAsFixed(6)}';
      _isLoading = false;
    }
    _initLocation();
  ...
  Future<void> _initLocation() async {
    ...
    if (widget.autoLocateOnInit || widget.initialCoordinate == null) {
  ...
  Future<void> _useCurrentLocation() async {
    ...
    final coordinate = _currentPosition!;
